...
{
    public class LigneAnnexeSeptImportRepository : ILigneAnnexeSeptImportRepository//, IAnnexeImportService<LigneAnnexeSeptImportView>
    {
        public IEnumerable<LigneAnnexe7ImportView> GetAll(string source)
        {
            ...
            using (var reader = new StreamReader(source, Encoding.GetEncoding(1252)))
            {
                using (var csv = new CsvReader(reader))
                {
                    ...
                    // register mapping
                    config.RegisterClassMap<LigneAnnexeSeptImportMap>();
                    config.ThrowOnBadData = true;
                    // skip empty row
                    config.SkipEmptyRecords = true;
                    // read csv file
                    var result = csv.GetRecords<LigneAnnexe7ImportView>().ToList();
