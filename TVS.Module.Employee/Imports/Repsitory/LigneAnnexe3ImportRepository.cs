...
{
    public class LigneAnnexe3ImportRepository : ILigneAnnexeTroisImportRepository
    {
        public IEnumerable<LigneAnnexe3ImportView> GetAll(string source)
        {
            ...
            using (var reader = new StreamReader(source, Encoding.GetEncoding(1252)))
            {
                using (var csv = new CsvReader(reader))
                {
                    ...
                    // register mapping
                    config.RegisterClassMap<LigneAnnexeTroisImportMap>();
                    config.ThrowOnBadData = true;
                    // skip empty row
                    config.SkipEmptyRecords = true;
                    // read csv file
                    var result = csv.GetRecords<LigneAnnexe3ImportView>().ToList();
