...
{
    public class LigneAnnexe1ImportRepository : ILigneAnnexeUnImportRepository
    {
        public IEnumerable<LigneAnnexe1ImportView> GetAll(string source)
        {
            ...
            using (var reader = new StreamReader(source, Encoding.GetEncoding(1252)))
            {
                using (var csv = new CsvReader(reader))
                {
                    try
                    {
                        ...
                        // register mapping
                        config.RegisterClassMap<LigneAnnexeUnImportMap>();
                        config.ThrowOnBadData = true;
                        // skip empty row
                        config.SkipEmptyRecords = true;
                        // read csv file
                        var result = csv.GetRecords<LigneAnnexe1ImportView>().ToList();
