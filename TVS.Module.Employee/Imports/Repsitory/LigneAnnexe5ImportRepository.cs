...
{
    public class LigneAnnexe5ImportRepository : ILigneAnnexeCinqImportRepository
    {
        public IEnumerable<LigneAnnexe5ImportView> GetAll(string source)
        {
            ...
            using (var reader = new StreamReader(source, Encoding.GetEncoding(1252)))
            {
                using (var csv = new CsvReader(reader))
                {
                    ...
                    // register mapping
                    config.RegisterClassMap<LigneAnnexeCinqImportMap>();
                    config.ThrowOnBadData = true;
                    // skip empty row
                    config.SkipEmptyRecords = true;
                    // read csv file
                    var result = csv.GetRecords<LigneAnnexe5ImportView>().ToList();
