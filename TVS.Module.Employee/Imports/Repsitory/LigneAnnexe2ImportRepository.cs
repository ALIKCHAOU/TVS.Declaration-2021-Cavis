...
{
    public class LigneAnnexe2ImportRepository : ILigneAnnexeDeuxImportRepository
    {
        public IEnumerable<LigneAnnexe2ImportView> GetAll(string source)
        {
            ...
            using (var reader = new StreamReader(source, Encoding.GetEncoding(1252)))
            {
                using (var csv = new CsvReader(reader))
                {
                    try
                    {
                        ...
                        // register mapping
                        config.RegisterClassMap<LigneAnnexeDeuxImportMap>();
                        config.ThrowOnBadData = true;
                        // skip empty row
                        config.SkipEmptyRecords = true;
                        // read csv file
                        var result = csv.GetRecords<LigneAnnexe2ImportView>().ToList();
