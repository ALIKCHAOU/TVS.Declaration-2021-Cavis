...
{
    public class LigneAnnexe4ImportRepository : ILigneAnnexeQuatreImportRepository
    {
        public IEnumerable<LigneAnnexe4ImportView> GetAll(string source)
        {
            ...
            using (var reader = new StreamReader(source, Encoding.GetEncoding(1252)))
            {
                using (var csv = new CsvReader(reader))
                {
                    ...
                    // register mapping
                    config.RegisterClassMap<LigneAnnexeQuatreImportMap>();
                    config.ThrowOnBadData = true;
                    // skip empty row
                    config.SkipEmptyRecords = true;
                    // read csv file
                    var result = csv.GetRecords<LigneAnnexe4ImportView>().ToList();
